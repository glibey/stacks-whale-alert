import axios from 'axios';
import fs from 'fs';
import { TwitterApi } from 'twitter-api-v2';
import { DEFAULT_MIN_WHALE_AMOUNT, isLegacyBnsLookupEnabled } from './config.js';
import { generateWhaleAlertImage } from './image-generator.js';
import { createLabelResolver } from './labels.js';
import { matchesSubscription } from './subscriptions.js';
import { createStore } from './store.js';

const STACKS_API_URL = 'https://api.hiro.so/extended/v1/tx/';
const DEFAULT_FETCH_ATTEMPTS = [
  { limit: 20, unanchored: false, type: undefined },
  { limit: 10, unanchored: false, type: undefined },
  { limit: 10, unanchored: true, type: undefined },
  { limit: 10, unanchored: false, type: 'token_transfer' },
];

const knownLabels = {
  SP000000000000000000002Q6VF78: 'Stacks Protocol',
  SP1P72Z3704V2FEKBWRFM27M8MRWP240J05W6WRE: 'Binance',
  SP2788M6S8Y39NZZS0VTM1S7M9F9D67T5E7B78GNB: 'OKX',
};
const X_MAX_POST_LENGTH = 280;

export const createWhaleAlertService = ({
  minWhaleAmount = DEFAULT_MIN_WHALE_AMOUNT,
  fetchAttempts = DEFAULT_FETCH_ATTEMPTS,
} = {}) => {
  const seenTx = new Set();
  const store = createStore();
  let twitterClient = null;

  const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
  const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID;
  const legacyBnsLookupEnabled = isLegacyBnsLookupEnabled();
  const labelResolver = createLabelResolver({ staticLabels: knownLabels, store });

  let cachedPrice = null;
  let lastFetched = 0;

  const getTwitterClient = () => {
    if (twitterClient) {
      return twitterClient;
    }

    const credentials = {
      appKey: process.env.TWITTER_API_KEY,
      appSecret: process.env.TWITTER_API_SECRET,
      accessToken: process.env.TWITTER_ACCESS_TOKEN,
      accessSecret: process.env.TWITTER_ACCESS_SECRET,
    };

    if (!credentials.appKey || !credentials.appSecret || !credentials.accessToken || !credentials.accessSecret) {
      return null;
    }

    twitterClient = new TwitterApi(credentials);
    return twitterClient;
  };

  const getStxPrice = async () => {
    try {
      console.log('[price] Fetching STX price from CoinMarketCap');
      const response = await axios.get(
        'https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=STX',
        {
          headers: {
            'X-CMC_PRO_API_KEY': process.env.COINMARKETCAP_API_KEY,
          },
        }
      );
      const stxData = response?.data?.data?.STX;
      const priceObj = Array.isArray(stxData)
        ? stxData.find((item) => item.slug === 'stacks' && item.symbol === 'STX')
        : stxData;
      const price = priceObj?.quote?.USD?.price || null;
      console.log(`[price] Success: STX price = $${price}`);
      return price;
    } catch (error) {
      console.error('[price] Error fetching STX price:', error.response?.status, error.response?.data || error.message);
      return null;
    }
  };

  const getCachedStxPrice = async () => {
    const now = Date.now();
    if (!cachedPrice || now - lastFetched > 1_800_000) {
      console.log('[price] Cache miss or expired, refreshing price');
      cachedPrice = await getStxPrice();
      lastFetched = now;
    } else {
      console.log('[price] Cache hit, using cached STX price');
    }
    return cachedPrice;
  };

  const cleanupImage = (imagePath) => {
    if (!imagePath || !fs.existsSync(imagePath)) {
      return;
    }

    console.log(`[notify] Cleaning up generated image ${imagePath}`);
    fs.unlinkSync(imagePath);
  };

  const formatDeliveryError = (error) => {
    if (!error) {
      return 'Unknown error';
    }

    const status = error.response?.status;
    const data = error.response?.data;

    if (typeof data === 'string') {
      return status ? `${status} ${data}` : data;
    }

    if (data && typeof data === 'object') {
      return status ? `${status} ${JSON.stringify(data)}` : JSON.stringify(data);
    }

    return status ? `${status} ${error.message}` : error.message;
  };

  const buildXMessage = (message) => {
    if (message.length <= X_MAX_POST_LENGTH) {
      return message;
    }

    const lines = message.split('\n');
    const explorerLine = lines.find((line) => line.startsWith('🔗 Explorer:'));
    const bodyLines = lines.filter((line) => line !== explorerLine).filter(Boolean);
    const suffix = explorerLine ? `\n\n${explorerLine}` : '';
    const available = X_MAX_POST_LENGTH - suffix.length - 1;

    if (available <= 0) {
      return `${message.slice(0, X_MAX_POST_LENGTH - 1)}…`;
    }

    const trimmedBody = bodyLines.join('\n').slice(0, available).trimEnd();
    return `${trimmedBody}…${suffix}`;
  };

  const sendToDiscord = async (message, target, alert) => {
    await axios.post(target, {
      content: message,
      embeds: [
        {
          title: `${alert.classification} Alert`,
          description: `${alert.amount.toLocaleString()} STX moved on Stacks`,
          url: alert.explorerUrl,
          fields: [
            { name: 'From', value: alert.senderDisplay, inline: true },
            { name: 'To', value: alert.recipientDisplay, inline: true },
            { name: 'USD', value: alert.usdAmount, inline: true },
          ],
        },
      ],
    });
  };

  const sendToWebhook = async (target, alert) => {
    await axios.post(target, alert, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
  };

  const sendToX = async (message, imagePath) => {
    const client = getTwitterClient();
    if (!client) {
      console.log('[notify:x] Skipped: missing X credentials');
      return {
        channel: 'x',
        status: 'skipped',
        error: 'Missing X credentials',
      };
    }

    const xMessage = buildXMessage(message);
    if (xMessage !== message) {
      console.log(`[notify:x] Trimmed post from ${message.length} to ${xMessage.length} characters`);
    }

    if (imagePath) {
      console.log(`[notify:x] Uploading media to X from ${imagePath}`);
      const mediaId = await client.v1.uploadMedia(imagePath);
      await client.v2.tweet({
        text: xMessage,
        media: { media_ids: [mediaId] },
      });
      return {
        channel: 'x',
        status: 'delivered',
        messageLength: xMessage.length,
        usedMedia: true,
      };
    }

    console.log('[notify:x] Posting text-only message to X');
    await client.v2.tweet(xMessage);
    return {
      channel: 'x',
      status: 'delivered',
      messageLength: xMessage.length,
      usedMedia: false,
    };
  };

  const sendToTelegram = async (message, imagePath) => {
    if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_CHAT_ID) {
      console.log('[notify:telegram] Skipped: missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID');
      return {
        channel: 'telegram',
        status: 'skipped',
        error: 'Missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID',
      };
    }

    if (imagePath && fs.existsSync(imagePath)) {
      console.log(`[notify:telegram] Sending Telegram photo from ${imagePath}`);
      const telegramApiUrl = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendPhoto`;
      const formData = new FormData();
      formData.append('chat_id', TELEGRAM_CHAT_ID);
      formData.append('caption', message);
      formData.append('photo', new Blob([fs.readFileSync(imagePath)]), 'whale-alert.png');

      const response = await fetch(telegramApiUrl, {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) {
        const errorBody = await response.text();
        throw new Error(`Telegram sendPhoto failed: ${response.status} ${errorBody}`);
      }

      return {
        channel: 'telegram',
        status: 'delivered',
        usedMedia: true,
      };
    }

    console.log('[notify:telegram] Sending Telegram text-only message');
    const telegramApiUrl = `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`;
    await axios.post(telegramApiUrl, {
      chat_id: TELEGRAM_CHAT_ID,
      text: message,
      parse_mode: 'Markdown',
    });
    return {
      channel: 'telegram',
      status: 'delivered',
      usedMedia: false,
    };
  };

  const sendTwitterAndTelegram = async (message, imagePath = null) => {
    console.log('[notify] Starting delivery to X and Telegram');
    let xResult = {
      channel: 'x',
      status: 'pending',
    };
    let telegramResult = {
      channel: 'telegram',
      status: 'pending',
    };

    try {
      xResult = await sendToX(message, imagePath);
      if (xResult.status === 'delivered') {
        console.log('[notify:x] Success');
      }
    } catch (err) {
      xResult = {
        channel: 'x',
        status: 'failed',
        error: formatDeliveryError(err),
      };
      console.error('[notify:x] Error posting tweet:', xResult.error);
    }

    try {
      telegramResult = await sendToTelegram(message, imagePath);
      if (telegramResult.status === 'delivered') {
        console.log('[notify:telegram] Success');
      }
    } catch (err) {
      telegramResult = {
        channel: 'telegram',
        status: 'failed',
        error: formatDeliveryError(err),
      };
      console.error('[notify:telegram] Error posting Telegram message:', telegramResult.error);
    }

    if (
      imagePath &&
      xResult.status === 'delivered' &&
      (telegramResult.status === 'delivered' || telegramResult.status === 'skipped')
    ) {
      cleanupImage(imagePath);
    } else if (imagePath) {
      console.log('[notify] Keeping generated image because one or more deliveries did not succeed');
    }

    console.log('[notify] Delivery step finished');
    return [xResult, telegramResult];
  };

  const sendSubscriptionNotifications = async (alert) => {
    const subscriptions = await store.listSubscriptions();
    const matchingSubscriptions = subscriptions.filter((subscription) => matchesSubscription(subscription, alert));
    const attempts = [];

    for (const subscription of matchingSubscriptions) {
      for (const channel of subscription.channels || []) {
        const attempt = {
          subscriptionId: subscription.id,
          channelType: channel.type,
          target: channel.target,
          attemptedAt: new Date().toISOString(),
          status: 'pending',
        };

        try {
          if (channel.type === 'discord') {
            await sendToDiscord(alert.message, channel.target, alert);
          } else if (channel.type === 'webhook') {
            await sendToWebhook(channel.target, alert);
          } else {
            attempt.status = 'skipped';
            attempt.error = `Unsupported channel type: ${channel.type}`;
            attempts.push(attempt);
            continue;
          }

          attempt.status = 'delivered';
        } catch (error) {
          attempt.status = 'failed';
          attempt.error = error.response?.data || error.message;
        }

        attempts.push(attempt);
      }
    }

    return attempts;
  };

  const resolveBnsName = async (address) => {
    const knownLabel = await labelResolver.resolveLabel(address);
    if (knownLabel) {
      console.log(`[labels] Label hit for ${address}: ${knownLabel}`);
      return knownLabel;
    }

    if (!legacyBnsLookupEnabled) {
      console.log(`[labels] Skipping remote lookup for ${address}; local labels only`);
      return null;
    }

    try {
      console.log(`[labels] Resolving BNS for ${address} via legacy Hiro endpoint`);
      const { data } = await axios.get(`https://api.hiro.so/v1/addresses/stacks/${address}/names`);
      if (data.names && data.names.length > 0) {
        console.log(`[labels] Success: ${address} -> ${data.names[0]}`);
        return data.names[0];
      }
    } catch (err) {
      console.error('[labels] Error resolving legacy BNS:', err.response?.status, err.response?.data || err.message);
    }
    console.log(`[labels] No label found for ${address}`);
    return null;
  };

  const classifyTransaction = (amountStx) => {
    if (amountStx >= 500_000) return '🐳 Mega Whale';
    if (amountStx >= 250_000) return '🐋 Humpback Whale';
    if (amountStx >= 100_000) return '🦈 Shark';
    if (amountStx >= 50_000) return '🐬 Dolphin';
    return '🐠 Fish';
  };

  const fetchRecentTransactions = async () => {
    for (const attempt of fetchAttempts) {
      try {
        console.log(`[fetch] Attempt limit=${attempt.limit} unanchored=${attempt.unanchored} type=${attempt.type || 'any'}`);
        const { data } = await axios.get(STACKS_API_URL, {
          params: {
            order: 'desc',
            sort_by: 'block_height',
            exclude_function_args: true,
            limit: attempt.limit,
            unanchored: attempt.unanchored,
            ...(attempt.type ? { type: attempt.type } : {}),
          },
        });

        console.log(`[fetch] Success: received ${data.results?.length || 0} transactions`);
        return data.results || [];
      } catch (err) {
        const status = err.response?.status;
        const code = err.response?.data?.code;
        console.error('[fetch] Error fetching transactions:', status, err.response?.data || err.message);

        if (!(status === 500 && code === '57014')) {
          throw err;
        }

        console.log('[fetch] Retrying after Hiro statement timeout');
      }
    }

    throw new Error('Hiro recent transactions API timed out for all retry attempts');
  };

  const processTransaction = async (tx) => {
    const txId = tx.tx_id;
    if (seenTx.has(txId) || await store.hasSeenTransaction(txId)) {
      console.log(`[tx:${txId}] Skipped: already seen`);
      return;
    }

    console.log(`[tx:${txId}] Processing transaction`);
    const transfer = tx?.token_transfer;
    const sender = tx?.sender_address || transfer?.sender_address;
    const recipient = transfer?.recipient_address;

    if (!sender || !recipient) {
      console.log(`[tx:${txId}] Skipped: missing sender or recipient`);
      seenTx.add(txId);
      await store.markTransactionSeen(txId);
      return;
    }

    const amountStx = (transfer.amount || 0) / 1e6;
    console.log(`[tx:${txId}] Parsed amount=${amountStx} sender=${sender} recipient=${recipient}`);

    if (amountStx >= minWhaleAmount) {
      console.log(`[tx:${txId}] Whale threshold met, enriching transfer`);
      const [price, senderName, recipientName] = await Promise.all([
        getCachedStxPrice(),
        resolveBnsName(sender),
        resolveBnsName(recipient),
      ]);

      const usdAmount = price
        ? (amountStx * price).toLocaleString('en-US', { style: 'currency', currency: 'USD' })
        : '-';
      const classification = classifyTransaction(amountStx);
      const senderDisplay = senderName
        ? `${senderName} (${sender.slice(0, 6)}...${sender.slice(-4)})`
        : `${sender.slice(0, 6)}...${sender.slice(-4)}`;
      const recipientDisplay = recipientName
        ? `${recipientName} (${recipient.slice(0, 6)}...${recipient.slice(-4)})`
        : `${recipient.slice(0, 6)}...${recipient.slice(-4)}`;
      const explorerUrl = `https://explorer.stacks.co/txid/${txId}`;
      const message = `${classification} Alert! 🚨\n\n💰 ${amountStx.toLocaleString()} #STX (${usdAmount})\n\n📤 From: ${senderDisplay}\n📥 To: ${recipientDisplay}\n\n🔗 Explorer: ${explorerUrl}`;
      const persistedAlert = await store.appendAlert({
        txId,
        amount: amountStx,
        usdAmount,
        classification,
        sender,
        recipient,
        senderDisplay,
        recipientDisplay,
        message,
        explorerUrl,
        deliveryAttempts: [],
      });

      let imagePath = null;
      try {
        console.log(`[tx:${txId}] Generating whale alert image`);
        imagePath = await generateWhaleAlertImage({
          amount: amountStx,
          classification,
          usdAmount,
          sender: senderName || `${sender.slice(0, 8)}...`,
          recipient: recipientName || `${recipient.slice(0, 8)}...`,
        });
        console.log(`[tx:${txId}] Image generated: ${imagePath}`);
      } catch (err) {
        console.error(`[tx:${txId}] Error generating image:`, err.message);
      }

      if (!imagePath) {
        console.log(`[tx:${txId}] Continuing without image, text-only alert will be sent`);
      }

      console.log(`[tx:${txId}] Sending alert`);
      const primaryDeliveryAttempts = await sendTwitterAndTelegram(message, imagePath);
      const subscriptionDeliveryAttempts = await sendSubscriptionNotifications({
        id: persistedAlert.id,
        txId,
        amount: amountStx,
        usdAmount,
        classification,
        sender,
        recipient,
        senderDisplay,
        recipientDisplay,
        message,
        explorerUrl,
      });
      await store.updateAlert(persistedAlert.id, (currentAlert) => ({
        ...currentAlert,
        imageGenerated: Boolean(imagePath),
        deliveryAttempts: [
          ...primaryDeliveryAttempts.map((attempt) => ({
            attemptedAt: new Date().toISOString(),
            ...attempt,
          })),
          ...subscriptionDeliveryAttempts,
        ],
      }));
      console.log(`[tx:${txId}] Alert send flow finished`);
    } else {
      console.log(`[tx:${txId}] Skipped: below whale threshold (${amountStx} STX)`);
    }

    seenTx.add(txId);
    await store.markTransactionSeen(txId);
    console.log(`[tx:${txId}] Completed`);
  };

  const fetchTransfers = async () => {
    try {
      console.log('[run] Fetching latest STX transactions');
      const transactions = await fetchRecentTransactions();
      const tokenTransfers = transactions.filter((tx) => tx?.tx_type === 'token_transfer' || tx?.token_transfer);
      console.log(`[run] Filtered ${tokenTransfers.length} token transfers from ${transactions.length} recent transactions`);
      await Promise.all(tokenTransfers.map(processTransaction));
      console.log('[run] Transfer processing finished');
    } catch (err) {
      console.error('[run] Error fetching transactions:', err.response?.status, err.response?.data || err.message);
    }
  };

  return {
    fetchTransfers,
  };
};
